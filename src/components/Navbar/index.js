...
const Navbar = () => {
  ...
  const getLinkClassName = (path, currentPath) => {
    const baseClasses = "menu-item mr-3 font-medium hover:text-yellow-600 transition";
    const activeClasses = "text-yellow-600";

    return `${baseClasses} ${path === currentPath ? activeClasses : ''}`;
  };
    ...
    <StyledWrapper className="w-full site-navbar sticky top-0 z-10">
      <div>
        <header className="flex items-center justify-between py-4">
          ...
          <nav className={`flex flex-col md:flex-row ${isSmallSCreen && !menuOpen ? 'hidden' : ''}`}>
            <Link href="/manifesto" legacyBehavior>
              <a className={getLinkClassName('/manifesto', asPath)}>Manifesto</a>
            </Link>
            ...
            <Link href="/downloads" legacyBehavior>
              <a className="menu-item text-white bg-yellow-600 hover:bg-yellow-700 focus:ring-4 focus:ring-yellow-200 font-medium rounded-lg text-sm px-4 lg:px-5 py-2 lg:py-2.5 sm:mr-2 lg:mr-0 focus:outline-none transition">
                Download
              </a>
            </Link>
          </nav>
          <button className="toggle inline-block md:hidden w-8 h-8 text-slate-700 p-1" onClick={menuToggle}>
