...
const Navbar = () => {
  ...
  const getLinkClassName = (path, currentPath) => {
    const baseClasses =
      'menu-item mr-3 font-medium hover:text-yellow-600 transition';
    ...
    <StyledWrapper className="w-full site-navbar sticky top-0 z-10">
      <div>
        <header className="flex items-center justify-between py-4">
          ...
          <nav
            className={`flex flex-col md:flex-row ${isSmallSCreen && !menuOpen ? 'hidden' : ''}`}
          >
            <Link href="/manifesto" legacyBehavior>
              <a className={getLinkClassName('/manifesto', asPath)}>
                Manifesto
              </a>
            </Link>
            ...
            <Link href="/changelog" legacyBehavior>
              <a className={getLinkClassName('/changelog', asPath)}>
                Changelog
              </a>
            </Link>
          ...
          <button
            className="toggle inline-block md:hidden w-8 h-8 text-slate-700 p-1"
            onClick={menuToggle}
          >
            <svg
              fill="currentColor"
